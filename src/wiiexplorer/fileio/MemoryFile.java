...
public class MemoryFile implements FileBase
{
    ...
    public String readString(String encoding, int length) throws IOException
    {
        ...
        for (int i = 0; length == 0 || i < length; i++)
        {
            ...
            if (res == CoderResult.UNDERFLOW)
                ...
                break;  // this is an ugly hack. We shouldn't be ignoring errors like that.
                        // but it seems that the ASCII charset decoder is too dumb to not read more
                        // than one byte when meeting the NULL terminator, making it occasionally
                        // throw UNMAPPABLE/MALFORMED errors depending on what follows the string
                        // the same issue occured with Shift-JIS but perhaps that is normal, I don't
                        // know about that encoding's internals
            //    throw new IOException(String.format("Error while reading string @ %1$X", curPosition));
    ...
    public int writeString(String encoding, String val, int length) throws IOException
    {
        ...
        for (int i = 0; i < ((length > 0) ? length : val.length()); i++)
        {
            ...
            if (res != CoderResult.UNDERFLOW)
                throw new IOException(String.format("Error while writing string @ %1$X", curPosition));
    ...
    protected void autoExpand(int newend) throws IOException
    {
